...
def test_setup():
  ...
  TestName = "test_OrganizationChartElements"
  description = "This test scenario is to verify all the Elements present at Organization Chart page"
  TestResult = []
  TestResultStatus = []
  TestFailStatus = []
  FailStatus="Pass"
  TestDirectoryName = "test_OrganizationChartElements"
  global Exe
  Exe="Yes"
        ...
        try:
            ...
            try:
                PageTitle1 = driver.find_element_by_xpath(
                    "//h2[text()='Portal List']").text
                print(PageTitle1)
                assert PageTitle1 in Ptitle1, PageName + " not present"
                TestResult.append(PageName + " (Portal List) is present")
                TestResultStatus.append("Pass")
            except Exception:
                TestResult.append(PageName + " (Portal List) is not present")
                TestResultStatus.append("Fail")
            print()
            # ---------------------------------------------------------------------------------

            # ---------------------------Verify Presence of back button on Portal page-----------------------------
            ...
            try:
                ...
                TestResult.append(PageName + "  is present on portal page")
                TestResultStatus.append("Pass")
            except Exception:
                TestResult.append(PageName + " is not present on portal page")
                TestResultStatus.append("Fail")
            print()
            time.sleep(TimeSpeed)
            # ---------------------------------------------------------------------------------
            ...
            # ---------------------------Verify Presence of elements in Portal listing table-----------------------------
            inside = "Portal listing"
            # ---------------loop for Columns in table for Portal listing table----------
            ItemList = ["Action", "Full Name", "Email", "Last Updated"]
            print(len(ItemList))
            ItemPresent = []
            ItemNotPresent = []
